...
- 编码更改：
    ...
    - [x] 将`求`字的编码从`giyi`改为`fiyi`，二级简码`fi`
    - [x] 将`万`字的编码从`gqe`改为`dne`，二级简码`dn`
    - [x] 将`步`字的编码从`hhr`改为`hir`，二级简码`hi`
    - [x] 将含有`廴辶`字根的字末笔识别码改为里面字根的末笔
    - [ ] 将含有`犭礻衤⻊鱼`字根的词组编码改为跳过第二码
    - [x] 将`还`字的编码改为`dpi`，二级简码`dp`
- 字根更改：
    - [x] 将`几`从`W`键移至`M`键
    - [x] 将`力`从`E`键移至`L`键
    - [ ] 将`集字右上角`从`Y`键移至`O`键
...
---------------------------------------------------------------------------------
 ...
 -------------------------------------------------------------------------
   ...
   ---------------------------------------------------------
左边总计：15155，右边总计：11293，比例为 57%:43%
...
---------------------------------------------------------------------------------
 ...
 -------------------------------------------------------------------------
   ...
   ---------------------------------------------------------
左边总计：369896，右边总计：241738，比例为 60%:40%
```
本码表：
```
单字（以千为单位）：
---------------------------------------------------------------------------------
| Q-1.2 | W-1.3 | E-0.9 | R-0.8 | T-1.4 | Y-1.6 | U-1.0 | I-1.0 | O-0.6 | P-0.7 |
---------------------------------------------------------------------------------
 | A-1.2 | S-0.7 | D-1.2 | F-1.6 | G-2.1 | H-1.3 | J-1.1 | K-1.4 | L-0.6 |
 -------------------------------------------------------------------------
   | Z     | X-0.6 | C-0.7 | V-0.6 | B-0.6 | N-1.3 | M-0.9 |
   ---------------------------------------------------------
左边总计：15003，右边总计：11475，比例为 57%:43%

词组（以万为单位）：
---------------------------------------------------------------------------------
| Q-2.6 | W-3.6 | E-1.5 | R-2.8 | T-4.3 | Y-3.3 | U-2.8 | I-2.7 | O-1.4 | P-2.2 |
---------------------------------------------------------------------------------
 | A-2.7 | S-2.1 | D-3.3 | F-3.7 | G-4.1 | H-2.3 | J-1.9 | K-2.3 | L-1.5 |
 -------------------------------------------------------------------------
   | Z     | X-1.4 | C-1.4 | V-1.3 | B-1.6 | N-2.5 | M-1.8 |
   ---------------------------------------------------------
左边总计：365062，右边总计：246914，比例为 60%:40%
```
